			...
			if (!current_shot->latency)
			{
				...
				if (!current_shot->hurt_player)
				{
					...
					if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
					{
						...
						{
							current_shot->shot_info.result = crypt_str("Resolver");
							g_ctx.globals.missed_shots[current_shot->last_target] = g_ctx.globals.missed_shots[current_shot->last_target] + 1; //-V807
						...
						else
						{
								...
								eventlogs::get().add(crypt_str("Missed shot due to inaccuracy"));
						}
					}
				}
			}
