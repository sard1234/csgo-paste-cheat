...
void misc::watermark()
{
	...
	if (m_engine()->IsInGame())
	{
		...
		if (nci)
		{
			...
			watermark = VERSION + g_ctx.username + crypt_str(" | ") + std::to_string(g_ctx.globals.ping) + crypt_str(" ms | ") + g_ctx.globals.time;
		}
	}

	auto box_width = render::get().text_width(fonts[NAME], watermark.c_str()) + 10;

	//render::get().gradient(width - 10 - box_width, 10, box_width / 2, 1, Color(117, 36, 209, GRADIENT_HORIZONTAL));
...
void misc::ChatSpamer()
{
	...
	{ 
		crypt_str("DoubleTap.Space, CSGO Private Cheat"),
		crypt_str("DoubleTap.Space, Best Private Cheat"),
		crypt_str("DoubleTap.Space, Shitting on NN dogs"),
		crypt_str("Niggas in my asshole"),
	};
