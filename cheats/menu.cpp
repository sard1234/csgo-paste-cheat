...
std::string get_config_dir()
{
	...
	if (SUCCEEDED(SHGetFolderPath(NULL, 0x001a, NULL, NULL, path)))
		folder = std::string(path) + crypt_str("\\DoubleTap.Space\\Configs\\");
...
void add_config()
{
	...
	if (g_cfg.new_config_name.find(crypt_str(".doubletap")) == std::string::npos)
		g_cfg.new_config_name += crypt_str(".doubletap");
...
bool LabelClick(const char* label, bool* v, const char* unique_id)
{
	...
	if (*v)
		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(136 / 255.f, 0 / 255.f, 255 / 255.f, 0.9f));
...
void lua_edit(std::string window_name)
{
	...
	auto get_dir = [&]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_PERSONAL, NULL, NULL, path)))
			file_path = std::string(path) + crypt_str("\\DoubleTap.Space\\Scripts\\");
	...
	if (!rage_section)
	{
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE2_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				//ImGui::Checkbox(crypt_str("Freestand (broken)"), &g_cfg.antiaim.freestand);
	...
	if (!child)
	{
		if (!misc_section)
		{
			...
			ImGui::BeginChild(crypt_str("##MISC1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				//ImGui::Checkbox(crypt_str("Show Keybinds"), &g_cfg.misc.draw_keys);
	...
	else
	{
		if (!misc_section)
		{
			...
			ImGui::BeginChild(crypt_str("##MISC2_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				ImGui::Checkbox(crypt_str("Bunnyhop"), &g_cfg.misc.bunnyhop);
				...
				ImGui::BeginChild(crypt_str("##LUA_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
				{
					...
					if (ImGui::CustomButton(crypt_str("Open scripts folder"), crypt_str("##LUAS__FOLDER"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
					{
						...
						auto get_dir = [&folder]() -> void
						{
							...
							if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_PERSONAL, NULL, NULL, path)))
								folder = std::string(path) + crypt_str("\\DoubleTap.Space\\Scripts\\");
...
void c_menu::draw(bool is_open)
{
	...
	// start menu render
	ImGui::Begin(crypt_str("DoubleTap.Space"), nullptr, ImGuiWindowFlags_NoDecoration);
	{
		...
		{																			 // KEYWORD : TABS_FIND
			...
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Rage"), ico_menu, crypt_str("C"), active_tab == 0, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 0;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Legit"), ico_menu, crypt_str("D"), active_tab == 1, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 1;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Visuals"), ico_menu, crypt_str("E"), active_tab == 2, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 2;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Players"), ico_menu, crypt_str("H"), active_tab == 3, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 3;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Misc"), ico_menu, crypt_str("F"), active_tab == 4, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 4;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Settings"), ico_menu, crypt_str("A"), active_tab == 5, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 5;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Scripts"), ico_bottom, crypt_str("J"), active_tab == 6, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 6;
			//if (ImGui::MainTab(crypt_str("Skins"), ico_menu, crypt_str("G"), active_tab == 7, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 7;
		...
		ImGui::GetWindowDrawList()->AddRectFilled(ImGui::GetCursorPos() + p + ImVec2(3 * dpi_scale, 0), ImGui::GetCursorPos() + p + ImVec2(4 * dpi_scale, 479 * dpi_scale), ImGui::ColorConvertFloat4ToU32(ImVec4(136 / 255.f, 0 / 255.f, 255 / 255.f, m_alpha * 0.3f))); //ImVec4(136 / 255.f, 0 / 255.f, 255 / 255.f, 0.9f);
			...
			ImGui::BeginGroup();  // groupping two childs to prevent multipadding
			{
				...
				else
				{
					ImGui::BeginChild(crypt_str("##content_first"), ImVec2(339 * dpi_scale, child_height * dpi_scale)); // first functional child
					{
						...
						case 5: draw_settings(0); break;
						case 6: draw_lua(0); break;
						//case 7: draw_skins(0); break;
						default: break;
						}
		...
		ImGui::SetCursorPos(ImGui::GetCursorPos() + ImVec2(850 * dpi_scale - ImGui::CalcTextSize(crypt_str("    DoubleTap.Space")).x - (11 * dpi_scale), (16 * dpi_scale) - (ImGui::CalcTextSize(crypt_str("    DoubleTap.Space")).y / 2)));
