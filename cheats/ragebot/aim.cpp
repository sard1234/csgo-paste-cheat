...
std::vector <scan_point> aim::get_points(adjust_data* record, int hitbox, bool from_aim)
{
	...
	else 
	{
		...
		if (g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].static_point_scale)
		{
				...
				scale2 = g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].body_scale;
		}
		else
		{
		...
		auto final_radiusbody = bbox->radius * scale2;
		...
		else if (hitbox >= HITBOX_PELVIS && hitbox <= HITBOX_UPPER_CHEST)
		{
			...
			points.emplace_back(scan_point(Vector(bbox->bbmax.x, bbox->bbmax.y, bbox->bbmax.z + final_radiusbody), hitbox,  false));
			points.emplace_back(scan_point(Vector(bbox->bbmax.x, bbox->bbmax.y, bbox->bbmax.z - final_radiusbody), hitbox,  false));

			points.emplace_back(scan_point(Vector(center.x, bbox->bbmax.y - final_radiusbody, center.z), hitbox, true));
		}
		else if (hitbox == HITBOX_RIGHT_CALF || hitbox == HITBOX_LEFT_CALF)
		{
			points.emplace_back(scan_point(center, hitbox, true));
			points.emplace_back(scan_point(Vector(bbox->bbmax.x - final_radiusbody, bbox->bbmax.y, bbox->bbmax.z), hitbox, false));
		}
		...
		{
			points.emplace_back(scan_point(center, hitbox, true));
			points.emplace_back(scan_point(Vector(bbox->bbmax.x + final_radiusbody, center.y, center.z), hitbox, false));
		}
	}
...
void aim::fire(CUserCmd* cmd)
{
	...
	static auto get_resolver_type = [](resolver_type type) -> std::string
	{
		...
		{
		case ORIGINAL:
			return crypt_str("original [58]");
		case ZARO:
			return crypt_str("zero [-58]");
		case FIRST2:
			return crypt_str("first [-17]");
		case SECOND2:
			return crypt_str("second [19]");
		case THIRD:
			return crypt_str("second [-19]");
		case LOW_FIRST:
			return crypt_str("low first [29]");
		case LOW_SECOND:
			return crypt_str("low second [-29]");
		}
	};
	...
	log << crypt_str("safepoint: ") + std::to_string((bool)final_target.data.point.safe) + crypt_str(" / ");
	log << crypt_str("bt tick: ") + std::to_string(backtrack_ticks) + crypt_str(" / ");
	log << crypt_str("resolver: ") + get_resolver_type(final_target.record->type) + std::to_string(final_target.record->side) + +crypt_str(" )");
